...
SELECT m.title, m.year
  FROM movie m
       INNER JOIN movie_inventory mi ON m.id = mi.movie_id
       LEFT JOIN movie_inventory_media mim ON mi.id = mim.movie_item_id AND mim.media_id > 1
       LEFT JOIN movie_inventory_media mim2 ON mi.id = mim2.movie_item_id AND mim2.media_id = 1
 WHERE mi.owner_id = 'a8117e44-c5b1-40d2-9572-81182a718d5f'
